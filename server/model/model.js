...
exports.createConnect = () => {
  ...
  if (conns.length === 1) {
    ...
    global.logger.log('info', 'Only one db connection, use Mongoose.connect()');
    ...
    global.logger.log('info', `${conns.length} db connections, use Mongoose.createConnection() for each`);
...
exports.disconnect = () => {
  mongoose.disconnect(() => {
    global.logger.log('info', 'mongoose disconnected');
  });
};
